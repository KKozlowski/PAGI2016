...
void SceneLoader::NextKeyframer(Scene *scene, Chunk *overchunk) {
	...
	while (overchunk->progress < overchunk->length) {
		...
		case LOCAL_POSITION:
			...
			fseek(file, temp.length - temp.progress - 12, SEEK_CUR);
			temp.progress = temp.length - 12;
			temp.progress += fread(&currentObject->transform.offset_deprecated.x, 1, 4, file);
			temp.progress += fread(&currentObject->transform.offset_deprecated.y, 1, 4, file);
			temp.progress += fread(&currentObject->transform.offset_deprecated.z, 1, 4, file);
			printSTR(currentObject->transform.offset_deprecated.to_string());
			temp.progress += ignore(temp.length - temp.progress);
			break;

...
{
	if (actor.parent != -1)
		ApplyTransformations2(scene.objects[actor.parent].transform);
	//printINT(actor.parent);
...
void prepareSling()
{
	auto l = scene.getObjectByName("L");
...
Object3DS* Scene::getObjectByName(string s)
{
	for (int i = 0; i < objects.size(); i++)
	{
		if (objects[i].name == s)
